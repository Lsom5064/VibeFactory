...
MAX_ATTACHMENT_IMAGE_PAYLOAD_BYTES = 4 * 1024 * 1024
MAX_ATTACHMENT_PDF_BYTES = 10 * 1024 * 1024
MAX_ATTACHMENT_TEXT_BYTES = 2 * 1024 * 1024
MAX_EXTRACTED_ATTACHMENT_TEXT_CHARS = 30000
...
def select_latest_usage_record(records: list[dict[str, Any]]) -> Optional[dict[str, Any]]:
...
def build_usage_payload(task: dict[str, Any], usage_record: Optional[dict[str, Any]]) -> dict[str, Optional[int]]:
    ...
    payload_source = usage_record or {}
    input_tokens = normalize_int(payload_source.get("input_tokens"))
    cached_input_tokens = normalize_int(payload_source.get("cached_input_tokens"))
    output_tokens = normalize_int(payload_source.get("output_tokens"))
    cached_output_tokens = normalize_int(payload_source.get("cached_output_tokens"))
    reasoning_output_tokens = normalize_int(payload_source.get("reasoning_output_tokens"))
    total_tokens = normalize_int(payload_source.get("total_tokens"))

    if input_tokens is None:
        input_tokens = normalize_int(task.get("input_tokens"))
    if cached_input_tokens is None:
        cached_input_tokens = normalize_int(task.get("cached_input_tokens"))
    if output_tokens is None:
        output_tokens = normalize_int(task.get("output_tokens"))
    if reasoning_output_tokens is None:
        reasoning_output_tokens = normalize_int(task.get("reasoning_output_tokens"))
    if total_tokens is None:
        total_tokens = normalize_int(task.get("total_tokens"))
...
def build_task_usage_snapshot(
    db: "Database",
    settings: "Settings",
    task: dict[str, Any],
) -> dict[str, Any]:
    task_id = str(task["task_id"])
    usage_record = select_latest_usage_record(db.list_task_usage_records(task_id))
    snapshot = fetch_codex_rate_limits(settings.codex_command)
    limit_name = normalize_whitespace(str(snapshot.limit_name or task.get("app_name") or "Codex")) or "Codex"
    return {
        "task_id": task_id,
        ...
        "usage": build_usage_payload(task, usage_record),
        "status": task.get("status") or "",
        "status_message": sanitize_user_visible_text(str(task.get("message") or "")),
    }
...
def normalize_attachment_name(value: Optional[str]) -> str:
    normalized = normalize_whitespace(str(value or ""))
    if not normalized:
        return "attachment"
    normalized = normalized.replace("\\", "/").split("/")[-1].strip()
...
def normalize_attachment_mime_type(value: Optional[str]) -> str:
    return normalize_whitespace(str(value or "")).lower()[:120]


def decode_attachment_base64(value: Optional[str]) -> bytes:
    normalized = normalize_reference_image_base64(value)
    if not normalized:
        return b""
    try:
        return base64.b64decode(normalized, validate=False)
    except (ValueError, binascii.Error) as exc:
        raise HTTPException(status_code=400, detail="invalid attachment base64") from exc


def infer_attachment_type(mime_type: str, declared_type: Optional[str]) -> str:
    normalized_type = normalize_whitespace(str(declared_type or "")).lower()
    if normalized_type in {"image", "pdf", "text"}:
...
def extract_text_attachment(name: str, mime_type: str, data: bytes) -> str:
    if mime_type == "application/pdf":
        try:
            from pypdf import PdfReader
        except ImportError as exc:
            raise HTTPException(status_code=500, detail="pypdf is required for PDF attachments") from exc
        reader = PdfReader(io.BytesIO(data))
        parts: list[str] = []
        for page in reader.pages:
            if sum(len(part) for part in parts) >= MAX_EXTRACTED_ATTACHMENT_TEXT_CHARS:
                break
    ...
    return text[:MAX_EXTRACTED_ATTACHMENT_TEXT_CHARS]


def normalize_attachments_payload(
    ...
    *,
    ...
    if attachments:
        if len(attachments) > 1:
            raise HTTPException(status_code=400, detail="only one attachment is supported")
        attachment = attachments[0]
        mime_type = normalize_attachment_mime_type(attachment.mime_type)
        attachment_type = infer_attachment_type(mime_type, attachment.type)
        if not attachment_type:
            raise HTTPException(status_code=400, detail="unsupported attachment type")
        if attachment_type == "image" and not mime_type.startswith("image/"):
            raise HTTPException(status_code=400, detail="image attachment must use image/*")
        if attachment_type == "pdf" and mime_type != "application/pdf":
            raise HTTPException(status_code=400, detail="PDF attachment must use application/pdf")
        if attachment_type == "text" and not mime_type.startswith("text/"):
            raise HTTPException(status_code=400, detail="text attachment must use text/*")
        data = decode_attachment_base64(attachment.base64)
        max_bytes = {
            "image": MAX_ATTACHMENT_IMAGE_PAYLOAD_BYTES,
            "pdf": MAX_ATTACHMENT_PDF_BYTES,
            "text": MAX_ATTACHMENT_TEXT_BYTES,
        }[attachment_type]
        if len(data) > max_bytes:
            raise HTTPException(status_code=413, detail=f"{attachment_type} attachment too large")
        item: dict[str, Any] = {
            "type": attachment_type,
            "mime_type": mime_type,
            "name": normalize_attachment_name(attachment.name),
            "base64": normalize_reference_image_base64(attachment.base64),
            "size_bytes": len(data),
        }
        if attachment_type in {"pdf", "text"}:
            item["extracted_text"] = extract_text_attachment(item["name"], mime_type, data)
        normalized.append(item)
    elif reference_image_name and reference_image_base64:
        data = decode_attachment_base64(reference_image_base64)
        if len(data) > MAX_ATTACHMENT_IMAGE_PAYLOAD_BYTES:
            raise HTTPException(status_code=413, detail="image attachment too large")
        normalized.append(
            {
                "type": "image",
...
def attachment_text_context(attachments: list[dict[str, Any]]) -> str:
    text_items = [item for item in attachments if item.get("type") in {"pdf", "text"}]
    if not text_items:
        return ""
    sections = []
    for item in text_items:
        extracted = normalize_whitespace(str(item.get("extracted_text") or ""))
        if extracted:
            sections.append(f"### {item.get('name')}\n\n{extracted}")
    return "\n\n".join(sections)[:MAX_EXTRACTED_ATTACHMENT_TEXT_CHARS]
...
def attachment_metadata(attachments: Any) -> list[dict[str, Any]]:
    ...
    for item in attachments[:1]:
        ...
        metadata.append(
            {
                "type": normalize_whitespace(str(item.get("type") or "")),
                "mime_type": normalize_whitespace(str(item.get("mime_type") or "")),
                "name": normalize_attachment_name(str(item.get("name") or "")),
...
def run_spec_clarification_agent(
    ...
    *,
    ...
    context_payload = {
        ...
        "attachment_text_available": bool(normalize_whitespace(attachment_text)),
        "attachment_text": attachment_text[:MAX_EXTRACTED_ATTACHMENT_TEXT_CHARS],
    }
    schema = {
        "type": "object",
...
def render_prompt_md(task: dict[str, Any], settings: Settings) -> str:
    ...
    attachments = attachment_metadata(conversation_state.get("attachments") or task.get("attachments"))
    attachment_text = normalize_whitespace(
        str(conversation_state.get("attachment_text") or task.get("attachment_text") or "")
    ...
    attachment_section = "- 첨부 파일 없음"
    if attachments:
        attachment_lines = [
            f"- [{item.get('type')}] {item.get('name')} ({item.get('mime_type')}, {item.get('size_bytes')} bytes)"
    ...
    *,
    ...
    with prompt_path.open("a", encoding="utf-8") as handle:
        ...
        if metadata:
            ...
            extracted = normalize_whitespace(attachment_text)[:MAX_EXTRACTED_ATTACHMENT_TEXT_CHARS]
            if extracted:
                handle.write(f"\n#### 서버 추출 텍스트\n\n{extracted}\n")
    ...
    def process_task(self, task_id: str) -> None:
        ...
        if self.settings.mock_codex:
            ...
            if not timed_out and not result_path.exists():
                codex_log_text = collect_task_logs(workspace_path, "logs/build.log")
...
def create_app() -> FastAPI:
    ...
    def generate(request: GenerateRequest) -> dict[str, Any]:
        ...
        requested_attachments = normalize_attachments_payload(
            request.attachments,
            reference_image_name=requested_reference_image_name,
            reference_image_base64=requested_reference_image_base64,
        )
        requested_attachment_text = attachment_text_context(requested_attachments)
        requested_image_attachment = next((item for item in requested_attachments if item.get("type") == "image"), None)
        if requested_image_attachment:
        ...
        if followup_task_id:
            ...
            effective_attachments = requested_attachments or attachment_metadata(previous_conversation_state.get("attachments"))
    ...
    def get_task_usage(
        ...
        if not is_task_access_allowed(task, device_id=device_id, phone_number=phone_number):
            raise HTTPException(status_code=404, detail="task not found")
        try:
            return build_task_usage_snapshot(db, settings, task)
        except Exception as exc:
            raise HTTPException(status_code=502, detail=f"usage unavailable: {exc}") from exc
